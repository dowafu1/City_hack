...
recognizer_pipe = None


def init_voice_recognizer():
  global recognizer_pipe
...
async def save_question_handler(m: types.Message, state: FSMContext):
  ...
  if not m.text or not m.text.strip():
    await m.answer("Вопрос не может быть пустым. Пожалуйста, напишите свой вопрос.")
    return
...
async def voice_input_to_text(message: types.Message):
  global recognizer_pipe

  # Проверяем, что recognizer_pipe инициализирован
  if recognizer_pipe is None:
    await message.answer("Сервис распознавания голоса временно недоступен.")
    return
  ...
  try:
    ...
    file_path = f'{path}/{voice.file_id}.ogg'
    await message.bot.download_file(voice.file_path, file_path)

    # Распознаем речь
    text = await recognize(file_path, recognizer_pipe)

    # Проверяем, что распознанный текст не None
    if text is not None and text.strip():
      await handle_ai_chat(message, another_text=text)
    else:
      await message.answer(
        "Извините, не удалось распознать голосовое сообщение. Попробуйте еще раз или отправьте текстовое сообщение.")

  except Exception as e:
    print(f"Ошибка при обработке голосового сообщения: {e}")
    await message.answer(
      "Произошла ошибка при обработке голосового сообщения. Попробуйте отправить текстовое сообщение.")
