...
def main_menu(user_id: int) -> types.InlineKeyboardMarkup:
    buttons = [
        ...
        [types.InlineKeyboardButton(text="💡 Совет дня", callback_data="tip")],
...
async def show_main(user_id: int, greeting: bool = False):
    text = "Привет снова!\nОсновные разделы — в меню ниже." if greeting else "Чем могу помочь?"
...
async def help_command(m: types.Message):
    ...
    await log_action(m.from_user.id, "help_command")
    help_text = (
        "📋 *Доступные команды:*\n\n"
        "📌 *Основные команды*\n"
        "/start – начать заново\n"
        ...
        "/admin – админ-панель\n\n"
        "💬 *Чат с ИИ*\n"
        "/stop – завершить диалог с ИИ\n\n"
        "📌 *Все остальные функции доступны через главное меню*"
    )
    await m.answer(help_text, parse_mode="Markdown")


async def menu_command(m: types.Message):
    """Обработчик команды /menu"""
    await log_action(m.from_user.id, "menu_command")
    await show_main(m.from_user.id)


async def sos_command(m: types.Message):
    """Обработчик команды /sos"""
    await log_action(m.from_user.id, "sos_command")
    await sos_direct(m)


async def admin_command(m: types.Message, state: FSMContext):
    """Обработчик команды /admin"""
    await log_action(m.from_user.id, "admin_command")
