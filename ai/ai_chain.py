        ...
        try:
            mistral_answer = await mistral_chat(
                ...
                preset_prompt='Ты эмпатичный психолог-профессионал, который помогает коллегам улучшать качество помощи клиентам. '
                              ...
                              f'Можешь добавить визуальные элементы (emoji), если считаешь это уместным. '
                              f'Еще у тебя есть теория, которая тебе может помочь разобраться с проблемой: {'\n'.join(context_data)}')
    ...
    try:
        total_answer = await sber_chat(
            ...
            preset_prompt=f'Ты эмпатичный психолог-профессионал, ведущий телеграмм-канал. '
                          ...
                          f'Твои предыдущие советы (разделены &): {"&".join(prev_tips) if len(prev_tips) else "советов еще не было"}.'
        )
        return total_answer
...
async def main():
    ...
    print(await chainize('Я не могу есть и смотреть на еду. Что мне делать?',
